...
function runAction(verb: string, hs: any) {
  ...
  let text: string;
  if (hs.needs && (verb === 'Open' || verb === 'Use')) {
    if (hs.needs.every((id: string) => hasItem(id))) {
      ...
      text = hs.responses?.[verb] || hs.responses?.Open || 'It opens.';
    } else {
      text = hs.needsBlocked || 'I still need something for this.';
    }
  } else if (verb === 'Pick up' && hs.pickup) {
    if (hs.pickupIf && !state.flags[hs.pickupIf]) {
      text = hs.pickupBlocked || "I can't take that right now.";
    } else {
      ...
      text = hs.responses?.['Pick up'] || 'Taken.';
    }
  } else if (verb === 'Look at') {
    text = hs.look || DEFAULT_RESPONSES['Look at'];
  } else {
...
function handleNPC(verb: string, npc: any) {
  ...
  if (verb === 'Talk to' && npc.dialogue) openDialogue(npc);
  else if (verb === 'Look at') say(npc.look);
  else say(DEFAULT_RESPONSES[verb] || "That won't help.");
}
...
function giveItem(npc: any, item: any) {
  ...
  if (!rule) { npcSays(npc, "Thanks, but I don't need that."); return; }
  if (rule.needAlso && !hasItem(rule.needAlso)) { npcSays(npc, rule.missing || "You're missing something."); return; }
  ...
  else if (rule.win) state.ending = { since: state.now, lines: ['CHAPTER COMPLETE', '', 'To be continued...'] };
}
...
function saveGame(silent = false) {
  try {
    ...
    if (!silent) flashSave('Game saved');
  } catch (e) { if (!silent) flashSave("Couldn't save"); }
}
function loadGame() {
  try {
    const raw = localStorage.getItem(SAVE_KEY);
    if (!raw) { flashSave('No save found'); return; }
    ...
    flashSave('Game loaded');
  } catch (e) { flashSave("Couldn't load"); }
}
function eraseGame() { try { localStorage.removeItem(SAVE_KEY); flashSave('Save erased'); } catch (e) { /* ignore */ } }
...
function drawTitle(t: number) {
  ...
  if (Math.sin(t * 3) > 0) centerText('Click to start', 172, 1, P.verbHot);
...
function drawSettings(ctx: CanvasRenderingContext2D) {
  ...
  ctr('SETTINGS', SETT.y + 5, P.verbHot);
  drawText(ctx, 'Music', SETT.x + 8, SETT.y + 18, P.inkLight, 1, P.black, 1);
  ...
  drawText(ctx, 'Sound: ' + (audio.isMuted() ? 'Off' : 'On'), SETT.x + 8, SOUND_Y(), P.inkLight, 1, P.black, 1);
  drawText(ctx, 'Game', SETT.x + 8, SETT.y + 94, P.inkLight, 1, P.black, 1);
  drawText(ctx, 'Save', SETT.x + 8, SAVE_Y(), P.verbIdle, 1, P.black, 1);
  drawText(ctx, 'Load', SETT.x + 58, SAVE_Y(), hasSave() ? P.verbIdle : P.panelWoodLit, 1, P.black, 1);
  drawText(ctx, 'Erase', SETT.x + 102, SAVE_Y(), P.verbIdle, 1, P.black, 1);
  drawText(ctx, '(i)', SETT.x + 150, SAVE_Y(), state.showSaveInfo ? P.verbHot : P.verbIdle, 1, P.black, 1);
  if (state.showSaveInfo) {
    const info = ['Your progress saves only in', 'this browser. It survives a', 'refresh: press Load to resume.', 'Erase removes it.'];
  ...
  drawText(ctx, 'About', SETT.x + 8, CLOSE_Y(), P.verbIdle, 1, P.black, 1);
  drawText(ctx, 'Close', SETT.x + SETT.w - 44, CLOSE_Y(), P.verbIdle, 1, P.black, 1);
}
...
function drawAbout(ctx: CanvasRenderingContext2D) {
  ...
  ctrA('Back', ABOUT.y + ABOUT.h - 11, P.verbHot);
}
